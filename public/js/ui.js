...
function ajaxreturngamenets() {
    $.ajax({
        ...
        data: { provinceId: localStorage.getItem('provinceId'), cityId: localStorage.getItem('cityId') },
    ...
    <div class="row w-100 p-3 m-0 popular justify-content-center ">
        ...
        <div class="col-lg-7 my-2 popular-img" style="background-image: url('${jsonData.best_gamenet.gamenet_image}')">
            ...
            <div class="row w-100 p-3 m-0  introduce ">
                ...
                <div
                    ...
                    <div class="w-100 d-flex mb-4 text-right ">
                        ...
                        <span class="text-white text-justify ">${jsonData.gamenets[0].address}<br/><hr class="text-white" />${jsonData.gamenets[0].description}</span>
                    </div>
                    <div class="row justify-content-center">
                    ${jsonData.btn_bs}
                    </div>
            ...
            $('.g-introduce2').empty();
            $('.g-introduce2').append(`<a href="gamenet/${jsonData.gamenets[1].gamenet_id}/${jsonData.gamenets[1].title}"></a>
            <div class="row w-100 p-3 m-0  introduce introduce1 ">
                <div class="col-lg-6 d-flex flex-column align-items-center justify-content-center  introduce2-data">
                    ...
                    <div class="w-100 d-flex mb-4 text-right ">
                        ...
                        <span class="text-white text-justify">${jsonData.gamenets[1].address}<br/><hr class="text-white" />${jsonData.gamenets[1].description}</span>
                    </div>
                    <div class="row justify-content-center">
                    ${jsonData.btn_bs}
                    </div>
            ...
            $('.g-introduce3').empty();
            $('.g-introduce3').append(`<a href="gamenet/${jsonData.gamenets[2].gamenet_id}/${jsonData.gamenets[2].title}"></a>
            <div class="row w-100 p-3 m-0  introduce introduce1 ">
                <div class="col-lg-6 d-flex flex-column align-items-center justify-content-center  introduce2-data">
                    ...
                    <div class="w-100 d-flex mb-4 text-right ">
                        ...
                        <span class="text-white text-justify">${jsonData.gamenets[2].address}<br/><hr class="text-white" />${jsonData.gamenets[2].description}</span>
                    </div>
                    <div class="row justify-content-center">
                    ${jsonData.btn_bs}
                    </div>
        ...
        <div class="row w-100 p-3 m-0  introduce ">
            ...
            <div
                ...
                <div class="w-100 d-flex mb-4 text-right ">
                    ...
                    <span class="text-white text-justify ">${jsonData.gamenets[3].address}<br/><hr class="text-white" />${jsonData.gamenets[3].description}</span>
                </div>
                <div class="row justify-content-center">
                ${jsonData.btn_bs}
                </div>
            ...
            $('.stars').starRating({ starSize: 25, readOnly: true });
        }
    });
}
$(document).ready(function() {
    if (localStorage.getItem('cityId') == null) { $('.pr-title').text('استان ها'); } else {
        let cityname = localStorage.getItem('city');
        let provincename = localStorage.getItem('province');
        $('.pr-title').text(provincename + ',' + cityname);
        ajaxreturngamenets();
    }
    window.onclick = function() { $(".Provinces").removeClass("show"); }
    $.ajax({
        url: urlprovinces,
        DataType: 'json',
        success: function(data) {
...
function getCities(ostanID) {
    ...
    $('.Provinces').append(`
        <div class="row text-right m-0 p-1 back">
...
$(document).on('click', '.Provinces .back',
    function(e) {
        e.stopPropagation();
        e.preventDefault();
        getprovinces();
